...
#define LINKNAME_IOF             L"\\DosDevices\\NDISLWFIO"
#define NTDEVICE_IOF             L"\\Device\\NDISLWFIO"
...
--*/
{
	...
	do {
		...
		if (Status != NDIS_STATUS_SUCCESS) {
			DEBUGP(DL_TRACE, "Register filter driver failed.\n");
			break;
		}
	...
	//
	NTSTATUS ntStatus;
	UNICODE_STRING ntUnicodeString; // NTDEVICE_STRING L"\\Device\\NDISLWF"
	UNICODE_STRING ntWin32NameString; // LINKNAME_STRING L"\\DosDevices\\NDISLWF"
	PDEVICE_OBJECT deviceObject = NULL; // ptr to device object
	RtlInitUnicodeString(&ntUnicodeString, NTDEVICE_IOF);
	RtlInitUnicodeString(&ntWin32NameString, LINKNAME_IOF);
	ntStatus = IoCreateDevice(
		DriverObject, // Our Driver Object
		0, // We don't use a device extension
		&ntUnicodeString, // Device name L"\\Device\\NDISLWF"
	...
	if (!NT_SUCCESS(ntStatus)) {
		DEBUGP(DL_ERROR, "Couldn't create the device object\n");
		return ntStatus;

	}
	DEBUGP(DL_TRACE, "create device success\n");
	...
	if (!NT_SUCCESS(ntStatus))DEBUGP(DL_TRACE, "create device fail\n");
	DEBUGP(DL_TRACE, "create device success\n");
...
{
	DEBUGP(DL_TRACE, "SioctlCreateClose called\n");
...
NDIS_STATUS FilterRegisterOptions(
...
--*/
{
	...
	ASSERT(FilterDriverContext == FilterDriverObject);

	if ((NdisFilterDriverHandle != FilterDriverHandle) ||
...
--*/
{
	...
	do {
		ASSERT(FilterDriverContext == FilterDriverObject);
		if (FilterDriverContext != FilterDriverObject) {
		...
		pFilter->FilterModuleName.Length = pFilter->FilterModuleName.MaximumLength = AttachParameters->
			FilterModuleGuidName->Length;
		pFilter->FilterModuleName.Buffer = (PWSTR)((PUCHAR)pFilter + sizeof(MS_FILTER));
		NdisMoveMemory(
			pFilter->FilterModuleName.Buffer,
			AttachParameters->FilterModuleGuidName->Buffer,
			pFilter->FilterModuleName.Length
		);

		pFilter->MiniportFriendlyName.Length = pFilter->MiniportFriendlyName.MaximumLength = AttachParameters->
			BaseMiniportInstanceName->Length;
		pFilter->MiniportFriendlyName.Buffer = (PWSTR)((PUCHAR)pFilter->FilterModuleName.Buffer + pFilter->
...
--*/
{
	PMS_FILTER pFilter = FilterModuleContext;
	NDIS_STATUS Status;
	BOOLEAN bFalse = FALSE;
...
--*/
{
	...
	RtlInitUnicodeString(&uniWin32NameString, LINKNAME_IOF);
...
--*/
{
	...
	switch (Request->RequestType) {
		case NdisRequestMethod:
			OriginalRequest->DATA.METHOD_INFORMATION.OutputBufferLength = Request->DATA.METHOD_INFORMATION.
				OutputBufferLength;
...
ULONGLONG InTraffic = 0, OutTraffic = 0;
INT SessionCount = 0;
UCHAR LocalIpAddress[4] = {0, 0, 0, 0};

typedef struct {
	USHORT Id;
	ULONGLONG InTraffic;
	ULONGLONG OutTraffic;
} SessionInfo;
...
NTSTATUS SioctlDeviceControl(
	...
	const IO_STACK_LOCATION* irpSp = IoGetCurrentIrpStackLocation(Irp);
	const ULONG inBufLength = irpSp->Parameters.DeviceIoControl.InputBufferLength;
	const ULONG outBufLength = irpSp->Parameters.DeviceIoControl.OutputBufferLength;
	...
	switch (irpSp->Parameters.DeviceIoControl.IoControlCode) {
		case IOCTL_SIOCTL_METHOD_BUFFERED:
			...
			DEBUGP(DL_TRACE, "Called IOCTL_SIOCTL_METHOD_BUFFERED\n")
			PrintIrpInfo(Irp);
			...
			switch (inBuf[0]) {
				case 'e': {
					...
					RtlCopyBytes(outBuf, inBuf + 1, length);
					break;
				}
				case 'l': {
					length = 1;
					if (strcmp(inBuf + 1, "fatal") == 0)
						filterDebugLevel = DL_FATAL;
					else if (strcmp(inBuf + 1, "error") == 0)
						filterDebugLevel = DL_ERROR;
					else if (strcmp(inBuf + 1, "warn") == 0)
						filterDebugLevel = DL_WARN;
					else if (strcmp(inBuf + 1, "trace") == 0)
						filterDebugLevel = DL_TRACE;
					else if (strcmp(inBuf + 1, "info") == 0)
						filterDebugLevel = DL_INFO;
					else if (strcmp(inBuf + 1, "loud") == 0)
						filterDebugLevel = DL_LOUD;
					else {
						outBuf[0] = 0;
						break;
					}
				...
				case 'i': {
					...
					DEBUGP(
						DL_WARN, "%u.%u.%u.%u", LocalIpAddress[0],
						LocalIpAddress[1],
						LocalIpAddress[2],
						LocalIpAddress[3]
					)
				...
				case 't': {
					length = (sizeof(ULONGLONG) << 1) + 1;
					sprintf(outBuf, "%llu %llu", InTraffic, OutTraffic);
					break;
				}
				case 's': {
					memset(outBuf, 0, sizeof(CHAR) * outBufLength);
					sprintf(outBuf, "%d", SessionCount);
					for (int i = 0; i < SessionCount; ++i)
						sprintf(outBuf, " %d %llu %llu", Sessions[i].Id, Sessions[i].InTraffic, Sessions[i].OutTraffic);
			...
			Irp->IoStatus.Information = outBufLength < length ? outBufLength : length;
		...
		default:
			...
			DEBUGP(DL_ERROR, "ERROR: unrecognized IOCTL %x\n", irpSp->Parameters.DeviceIoControl.IoControlCode);
			break;
	}
...
int CheckNetBufferList(
	...
	LONG DataTotalLength = 0, DataCurrentLength = 0, DataMdlLength = 0, lOffset = 0, rOffset = 0;
	PUCHAR pDataCopyAddr, pDataMdlAddr;

	do {
		PNET_BUFFER pCurrentNetBuffer = NET_BUFFER_LIST_FIRST_NB(NetBufferLists);
		DataTotalLength = pCurrentNetBuffer->DataLength;
		pDataCopyAddr = NdisAllocateMemoryWithTagPriority(
			pFilter->FilterHandle, DataTotalLength, FILTER_ALLOC_TAG, LowPoolPriority
		);
		if (NULL == pDataCopyAddr) {
			DEBUGP(DL_WARN, "cannot allocate NdisAllocateMemoryWithTagPriority\n")
			return 1;
		}
		NdisZeroMemory(pDataCopyAddr, DataTotalLength);

		PMDL pMdl = NET_BUFFER_CURRENT_MDL(pCurrentNetBuffer); // get current mdl
		NdisQueryMdl(pMdl, &pDataMdlAddr, &DataMdlLength, NormalPagePriority); // get information

		// lOffset: the left offset of data in current mdl
		// rOffset: the right border of data in current mdl
		// note that the rOffset maybe less than the dataLength of the current mdl
		lOffset = NET_BUFFER_CURRENT_MDL_OFFSET(pCurrentNetBuffer);
		rOffset = min(DataTotalLength, DataMdlLength - lOffset);

		// move memory
		NdisMoveMemory(pDataCopyAddr, pDataMdlAddr + lOffset, rOffset);
		// update he current copied data length
		DataCurrentLength = DataCurrentLength + rOffset;

		// if current copied data length is less than total data length, then continue copying
		while (DataCurrentLength < DataTotalLength) {
			pMdl = pMdl->Next;
			NdisQueryMdl(pMdl, &pDataMdlAddr, &DataMdlLength, NormalPagePriority);

			rOffset = min(DataMdlLength, DataTotalLength - DataCurrentLength);
			NdisMoveMemory(pDataCopyAddr + DataCurrentLength, pDataMdlAddr, rOffset);
			DataCurrentLength = DataCurrentLength + rOffset;
		}

	} while (bFalse);

	if (Mode == CNBL_SEND || Mode == CNBL_RECEIVE) {
		const CHAR* direction = Mode == CNBL_SEND ? "Send" : "Receive";
		ETHERNET_HEADER* EH = (ETHERNET_HEADER*)pDataCopyAddr;
		IPV4_HEADER* IPv4H = (IPV4_HEADER*)(EH + 1);
		if (memcmp(
			Mode == CNBL_SEND ? IPv4H->SourceAddress : IPv4H->DestinationAddress,
			LocalIpAddress,
			sizeof(CHAR) * 4
		) != 0)
			goto End;
		DEBUGP(
			DL_WARN, "%s IP { Protocol: %d, SrcIP: %u.%u.%u.%u, DestIP: %u.%u.%u.%u }",
			direction,
			IPv4H->Protocol,
			IPv4H->SourceAddress[0],
			IPv4H->SourceAddress[1],
			IPv4H->SourceAddress[2],
			IPv4H->SourceAddress[3],
			IPv4H->DestinationAddress[0],
			IPv4H->DestinationAddress[1],
			IPv4H->DestinationAddress[2],
			IPv4H->DestinationAddress[3]
		)
		if (IPv4H && IPv4H->Protocol == IPHP_UDP) {
			UDP_HEADER* UDPH = (UDP_HEADER*)(IPv4H + 1);
			DEBUGP(
				DL_WARN, "%s UDP { SrcPort: %d, DstPort: %d }",
				direction, UDPH->SourcePort, UDPH->DestinationPort
			)
			if ((Mode == CNBL_SEND ? UDPH->SourcePort : UDPH->DestinationPort) == L2TP_PORT) {
				L2TP_HEADER* L2TPH = (L2TP_HEADER*)(UDPH + 1);
				DEBUGP(
					DL_WARN, "%s L2TP { Type: %s, Length: %d, TunnelId: %d, SessionId: %d }",
					direction,
					L2TPH->FlagsAndVersion & Control ? "Control" : "Data",
					RtlUshortByteSwap(L2TPH->Length),
					RtlUshortByteSwap(L2TPH->TunnelId),
					RtlUshortByteSwap(L2TPH->SessionId)
				)
				if ((L2TPH->FlagsAndVersion & Control) == 0) {
					USHORT id = L2TPH->SessionId;
					...
					if (session == NULL) {
						...
						*session = (SessionInfo){id, 0ll, 0ll};
					}
					if (Mode == CNBL_SEND) {
						OutTraffic += L2TPH->Length;
						session->OutTraffic += L2TPH->Length;
					}
					else {
						InTraffic += L2TPH->Length;
						session->InTraffic += L2TPH->Length;
					}
				}
			}
		}
...
End:
	...
	NdisFreeMemoryWithTagPriority(pFilter->FilterHandle, pDataCopyAddr, FILTER_ALLOC_TAG);

	// return 1 to pass this package, return 0 to block this package
	//
	// 2. RETURN VALUE IS USED
	//
	return 1;
}
...
--*/
{
	...
	//PNET_BUFFER_LIST    DropListHead = NULL;
	...
	do {
		...
		if (pFilter->TrackSends) {
			...
			Pass = CheckNetBufferList(pFilter, NetBufferLists, CNBL_SEND);
			if (Pass)
			...
			//DEBUGP(DL_WARN, "NB  %d\n", NetBufferLists->FirstNetBuffer->DataLength);
			//DEBUGP(DL_WARN, "MDL %d\n", NetBufferLists->FirstNetBuffer->CurrentMdl->ByteCount - NetBufferLists->FirstNetBuffer->CurrentMdlOffset);
...
--*/
{
	...
	BOOLEAN bFalse = FALSE;
	int Pass = 1;
	PNET_BUFFER_LIST pNBLTemp, pNBLCurrent;
#if DBG
	ULONG ReturnFlags;
#endif
	...
	do {
		...
		if (pFilter->TrackReceives) {
			...
			//FILTER_RELEASE_LOCK(&pFilter->Lock, DispatchLevel);
			pNBLTemp = NetBufferLists;
			while (pNBLTemp) {
				Pass = CheckNetBufferList(pFilter, pNBLTemp, CNBL_RECEIVE);
				pNBLCurrent = pNBLTemp;
				pNBLTemp = pNBLTemp->Next;
				pNBLCurrent->Next = NULL;
				if (Pass)
					NdisFIndicateReceiveNetBufferLists(
						pFilter->FilterHandle,
						pNBLCurrent,
					...
					if (NDIS_TEST_RECEIVE_AT_DISPATCH_LEVEL(ReceiveFlags))
						NDIS_SET_RETURN_FLAG(ReturnFlags, NDIS_RETURN_FLAGS_DISPATCH_LEVEL);
					NdisFReturnNetBufferLists(pFilter->FilterHandle, pNBLCurrent, ReturnFlags);
				}
			}
...
NDIS_STATUS filterDoInternalRequest(
...
VOID filterInternalRequestComplete(
...
VOID PrintIrpInfo(PIRP Irp) {
	...
	DEBUGP(DL_TRACE, "\tIrp->AssociatedIrp.SystemBuffer = 0x%p\n", Irp->AssociatedIrp.SystemBuffer);
	DEBUGP(DL_TRACE, "\tIrp->UserBuffer = 0x%p\n", Irp->UserBuffer);
	DEBUGP(
		DL_TRACE, "\tirpSp->Parameters.DeviceIoControl.Type3InputBuffer = 0x%p\n",
		irpSp->Parameters.DeviceIoControl.Type3InputBuffer
	);
	DEBUGP(
		DL_TRACE, "\tirpSp->Parameters.DeviceIoControl.InputBufferLength = %d\n",
		irpSp->Parameters.DeviceIoControl.InputBufferLength
	);
	DEBUGP(
		DL_TRACE, "\tirpSp->Parameters.DeviceIoControl.OutputBufferLength = %d\n",
