...
PFILTERD_ALLOCATION filterdMemoryHead = NULL;
PFILTERD_ALLOCATION filterdMemoryTail = NULL;
...
VOID filterAuditShutdown(
	VOID
) {
	if (filterdInitDone) {
		if (filterdAllocCount != 0) {
			DEBUGP(
				DL_ERROR, "AuditShutdown: unfreed memory, %d blocks!\n",
				filterdAllocCount
			);
			DEBUGP(
				DL_ERROR, "MemoryHead: 0x%p, MemoryTail: 0x%p\n",
